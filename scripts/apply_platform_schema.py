"""Cria tabelas da plataforma no Supabase (requer SUPABASE_DB_URL no .env)."""
...
SQL_PATH = Path(__file__).resolve().parent.parent / "supabase" / "003_platform.sql"
...
def main():
    ...
    if not db_url:
        ...
        print("  Depois rode: python scripts/apply_platform_schema.py")
        sys.exit(1)
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(sql)
        print("Tabelas da plataforma criadas com sucesso.")
    finally:
        conn.close()
